...
function getPersistableGalleryInputDraft(state: AppState) {
  return state.appMode === 'gallery' ? saveGalleryInputDraft(state) : state.galleryInputDraft
}
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    a.href = url
    a.download = `gpt-image-2-backup_${formatExportFileTime(new Date(exportedAt))}.zip`
    a.click()
